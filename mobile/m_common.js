...
(function injectCommonStyles() {
    ...
    s.textContent = [
        ...
        '#modal-logout .modal-box{max-width:320px;padding:22px 24px;}',
        '#modal-logout h3{font-size:0.92rem;}',
        '#modal-logout p{margin:10px 0;font-size:0.82rem;}',
        '#modal-logout .btn-row{gap:8px;margin-top:16px;}',
        '#modal-logout .btn-row button{padding:9px;font-size:0.82rem;}',
...
function imtiOpenModal(id) {
    ...
    // 모달이 슬라이드 메뉴(z-index 높음)에 가려지지 않도록 메뉴를 닫는다
    if (typeof imtiCloseMenu === 'function') {
        imtiCloseMenu();
    } else {
        var _drawer = document.getElementById('nav-drawer');
        var _backdrop = document.getElementById('nav-drawer-backdrop');
...
function imtiLoadNavbar(currentPage, afterLoad) {
    ...
    function _run() {
        ...
        // If the navbar markup is already embedded in the page, don't fetch —
        // just activate it. This avoids blank navbars when fetch is blocked
        // (e.g. opening the file locally) and is more robust overall.
        if (placeholder && placeholder.querySelector('.topnav')) {
            _activate();
            return;
        }
